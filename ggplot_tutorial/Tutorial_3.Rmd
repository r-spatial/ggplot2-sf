...
We now prepare the subplots, starting by the world map with a rectangle around the Gulf of Mexico, using `geom_rect`, (see Section 1 and 2 for the details of how to prepare this map). `geom_rect` uses spatial coordinates to create the rectangle on the map, specifying the `xmin`, `xmax`,`ymin`, and `ymax`:
...
The second map is very similar, but centered on the Gulf of Mexico (using `coord_sf`). `scale_fill_viridis` is used to create a color scheme for the map, which will override the default colors of `fill=REGION`:
...
As we want to connect the two subplots to main map using arrows, the coordinates of the two arrows will need to be specified before plotting. We prepare a `data.frame` storing start and end coordinates (`x1`, starting point, and `x2`, end point, on the x-axis, `y1`, starting point, and `y2`, ending point, on the y-axis):
